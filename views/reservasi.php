...
//pagination & filter
$no_hal = isset($_GET['page_num']) ? (int)$_GET['page_num'] : 1;
if ($no_hal < 1) $no_hal = 1;
...
try {
    ...
    if ($no_hal > $total_hal && $total_hal > 0) {
        $no_hal = $total_hal;
        $offset = ($no_hal - 1) * $data_per_hal;
    }
    ...
    $data_sql = "
        ...
        LIMIT $data_per_hal OFFSET $offset
    ";
...
<div class="card table-card">
    <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <span class="badge bg-primary"><?php echo $total_data; ?> Reservasi</span>
    </div>
    <div class="card-body">
        <?php if (!empty($reservasi_list)): ?>
            <div class="table-responsive">
                <table class="table table-hover">
                    ...
                    <tbody>
                        <?php foreach ($reservasi_list as $reserv): ?>
                        <tr>
                            <td><strong>#<?php echo htmlspecialchars($reserv['id_reservasi'] ?? 'N/A'); ?></strong></td>
                            <td>
                                <?php echo htmlspecialchars($reserv['nama_tamu'] ?? 'N/A'); ?><br>
                                <small class="text-muted"><?php echo htmlspecialchars($reserv['no_telp'] ?? ''); ?></small>
                            </td>
                            <td>
                                <strong><?php echo htmlspecialchars($reserv['nomor_kamar'] ?? 'N/A'); ?></strong><br>
                                <small class="text-muted"><?php echo htmlspecialchars($reserv['nama_tipe'] ?? 'N/A'); ?></small>
                            </td>
                            ...
                            <td><?php echo isset($reserv['lama_menginap']) ? $reserv['lama_menginap'] . ' malam' : 'N/A'; ?></td>
                            ...
                            <td><?php echo isset($reserv['status_bayar']) ? status_badge($reserv['status_bayar']) : 'N/A'; ?></td>
                            <td>
                                <?php if (($reserv['status_reservasi'] ?? '') == 'pending'): ?>
                                    <a href="?page=reservasi&action=confirm&id=<?php echo $reserv['id_reservasi']; ?>" 
                                       class="btn btn-sm btn-success btn-action"
                                       onclick="return confirm('Konfirmasi reservasi ini?')">
                                        <i class="bi bi-check"></i> Confirm
                                    </a>
                                    <a href="?page=reservasi&action=cancel&id=<?php echo $reserv['id_reservasi']; ?>" 
                                       class="btn btn-sm btn-danger btn-action"
                                       onclick="return confirm('Batalkan reservasi ini?')">
                                        <i class="bi bi-x"></i> Cancel
                                    </a>
                                <?php elseif (($reserv['status_reservasi'] ?? '') == 'confirmed'): ?>
                                    <span class="text-success">
                                        <i class="bi bi-check-circle"></i> Terkonfirmasi
                                    </span>
                                <?php elseif (($reserv['status_reservasi'] ?? '') == 'check-in'): ?>
            ...
            <nav class="mt-4">
                <ul class="pagination justify-content-center">
                    <li class="page-item <?php echo $no_hal <= 1 ? 'disabled' : ''; ?>">
                        <a class="page-link" href="?page=reservasi&page_num=<?php echo $no_hal - 1; ?>&search=<?php echo urlencode($search); ?>&filter_status=<?php echo urlencode($filter_status); ?>">
                            Previous
                        </a>
                    </li>
                    
                    <?php 
                    $start_hal = max(1, $no_hal - 2);
                    $end_hal = min($total_hal, $start_hal + 4);
                    $start_hal = max(1, $end_hal - 4);
                    
                    for ($i = $start_hal; $i <= $end_hal; $i++): 
                    ?>
